...
const SRC = [];
do {
  input = prompt("Enter the row and column value for start node, separated by a comma: ex- 9,9 | Dont enter beyond 19/49 ");
...
const DEST = [];
do {
  input2 = prompt("Enter the row and column value for end node, separated by a comma: ex- 9,40 | Dont enter beyond 19,49 ");

  if (!input2.includes(",")) {
    alert("Please enter two values");
  }
} while (!input.includes(","));
const [row2, col2] = input2.split(",").map(num => parseInt(num.trim()));
DEST.push(row2);
DEST.push(col2);
      ...
      if (ALGORITHM === "")
        ...
        window.onmousedown = (evt) => {
          ...
          if (target.classList.contains("box-container")) {
            ...
            document.querySelector(
              "#message"
            ).innerHTML = `${ALGORITHM} Algorithm | Weight Costs 10x More`;
          }
        };
      } else
        document.querySelector(
...
function clear() {
  ...
  // Clear Message
  document.querySelector("#message").innerHTML =
    "GUIDE : Click on Grid to Add Wall | Select Algorithm | Click with W Key Pressed to Add Weight";
}
